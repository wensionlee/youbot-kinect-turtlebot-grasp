...
#define MAXCLIENTS 3           //宏定义，最多2个客户端连接
...
/*子线程*/
static void * ProcessClientRequests_3() 
{
	 clntSock[2] = * ((int *) lpParam);	//这里需要强制转换，注意：指针类型的
	//char position_char[2];
	//position_char[0] = position[0];
	//position_char[1] = position[1];
	//char* msg = "Hello, my client.\r\n";
 ...
 /*
		...
		*/
		  //读取客户端传回的数据  
    		char buffer[3][100]; 
		read(clntSock[2], buffer[0], sizeof(buffer[0]));  
   	 	read(clntSock[2], buffer[1], sizeof(buffer[1]));  
    		read(clntSock[2], buffer[2], sizeof(buffer[2]));  
    		float bu[3];
    		bu[0]=atof(buffer[0]);
    		bu[1]=atof(buffer[1]);
    		bu[2]=atof(buffer[2]);
...
{  
	...
	if (j==0)
		...
		pthread_create(&threads[j], NULL, ProcessClientRequests_2, (void*)&clntSock[j]);  //启动新线程2，并且将socket传入
             else
		{
		pthread_create(&threads[2], NULL, ProcessClientRequests_3, (void*)&clntSock[2]);  //启动新线程3，并且将socket
			//底盘运动
			//float a=0.313651;
			//float b=0.309381;
			
			float baseX=bu[0]*0.001;
			float baseY=bu[1]*0.001;
			...
			ik(a-0.248,b-0.085,-0.05,0.6851, 0.1749, 0.6851, -0.1749);
